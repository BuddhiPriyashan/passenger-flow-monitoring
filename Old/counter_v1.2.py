...
def save_to_database(count):
    ...
    update_query = 'UPDATE passenger_count_epochtime SET count = count + %s, epoch_time = %s WHERE operator = %s AND vehicle_id = %s'
    ...
    mycursor.execute(update_query, (increment, eptime, record_id, vehicle_id))
    # Commit the changes to the database
    mydb.commit()


    update_query_2 = 'INSERT INTO passenger_count_epochtime_grafana (operator,vehicle_id,epoch_time,count) VALUES (%s,%s,%s,count + %s)'
    # Set the new value and the corresponding ID
    record_id = 'ASTRA GmbH'
    vehicle_id = 111222333
    increment = 1
    eptime = int(time.time())
    # Execute the update query
    mycursor.execute(update_query_2, (record_id, vehicle_id, eptime, increment))
    # Commit the changes to the database
    mydb.commit()
